...
func (_m *AccountMock) GetAccountProxies() config.AccountProxies {
	ret := _m.Called()

	var r0 config.AccountProxies
	if rf, ok := ret.Get(0).(func() config.AccountProxies); ok {
		r0 = rf()
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(config.AccountProxies)
		}
	}
