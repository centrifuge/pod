...
SubmitTransaction submits transaction to the ethereum chain. Uses Nonce from in memory account. Which is incremented
after successful send.
Blocking Function that sends transaction using reflection wrapped in a retrial block. Retrial may happen when error regarding nonce or price is identified.
- contractMethod: Contract Method that implements GenericEthereumAsset (usually autogenerated binding from abi)
- params: Arbitrary number of parameters that are passed to the function fname call
Note: contractMethod must always return "*types.Job, error"
*/
func (gc *gethClient) SubmitTransaction(contractMethod interface{}, opts *bind.TransactOpts, params ...interface{}) (*types.Transaction, error) {
	...
	for {
		if current >= maxTries {
			...
			return nil, errors.NewTypedError(ErrEthTransaction, err)
		}

		current++
		tx, err = gc.executeCall(contractMethod, opts, params)
		if err == nil {
			break
		}

		if strings.Contains(err.Error(), ErrIncNonce.Error()) || strings.Contains(err.Error(), ErrIncPrice.Error()) {
			log.Warningf("Concurrent transaction identified with err [%s] and used Nonce [%s], trying again [%d/%d]\n", err.Error(), opts.Nonce.String(), current, maxTries)
...
func (gc *gethClient) executeCall(contractMethod interface{}, opts *bind.TransactOpts, params []interface{}) (*types.Transaction, error) {
	var err error
	var tx *types.Transaction
