...
func TestService_SignVerify(t *testing.T) {
	...
	assert.NoError(t, err)
	attr, err := documents.NewSignedAttribute("funding_agreement[4].signatures[0]", testingidentity.GenerateRandomDID(), acc, model.ID(), model.NextVersion(), invalidValue, documents.AttrBytes)
	assert.NoError(t, err)
