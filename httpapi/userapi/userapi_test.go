...
func TestRegister(t *testing.T) {
	...
	assert.Equal(t, r.Routes()[4].Pattern, "/purchase_orders")
	assert.Len(t, r.Routes()[4].Handlers, 1)
	assert.NotNil(t, r.Routes()[4].Handlers["POST"])
	assert.Equal(t, r.Routes()[5].Pattern, "/purchase_orders/{document_id}")
	assert.Len(t, r.Routes()[5].Handlers, 2)
	assert.NotNil(t, r.Routes()[5].Handlers["GET"])
	assert.NotNil(t, r.Routes()[5].Handlers["PUT"])
	assert.Equal(t, r.Routes()[6].Pattern, "/purchase_orders/{document_id}/versions/{version_id}")
