...
func TestRegister(t *testing.T) {
	...
	assert.Equal(t, r.Routes()[8].Pattern, "/invoices/{document_id}/mint/unpaid")
	assert.NotNil(t, r.Routes()[8].Handlers["POST"])
	assert.Equal(t, r.Routes()[9].Pattern, "/invoices/{document_id}/versions/{version_id}")
	assert.NotNil(t, r.Routes()[9].Handlers["GET"])
	assert.Equal(t, r.Routes()[10].Pattern, "/purchase_orders")
	assert.Len(t, r.Routes()[10].Handlers, 1)
	assert.NotNil(t, r.Routes()[10].Handlers["POST"])
	assert.Equal(t, r.Routes()[11].Pattern, "/purchase_orders/{document_id}")
	assert.Len(t, r.Routes()[11].Handlers, 2)
	assert.NotNil(t, r.Routes()[11].Handlers["GET"])
	assert.NotNil(t, r.Routes()[11].Handlers["PUT"])
	assert.Equal(t, r.Routes()[12].Pattern, "/purchase_orders/{document_id}/versions/{version_id}")
	assert.NotNil(t, r.Routes()[12].Handlers["GET"])
	assert.Equal(t, r.Routes()[13].Pattern, "/relationships/{document_id}/entity")
