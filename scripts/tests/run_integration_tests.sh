...
export CENT_NETWORKS_TESTING_CONTRACTADDRESSES_IDENTITY=`cat $CENT_ETHEREUM_CONTRACTS_DIR/build/contracts/Identity.json | jq -r --arg NETWORK_ID "${NETWORK_ID}" '.networks[$NETWORK_ID].address' | tr -d '\n'`
