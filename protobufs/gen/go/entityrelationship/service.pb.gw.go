...
/*
Package entityrelationshipb is a reverse proxy.
...
var (
	filter_DocumentService_Get_0 = &utilities.DoubleArray{Encoding: map[string]int{"owner_identity": 0}, Base: []int{1, 1, 0}, Check: []int{0, 1, 2}}
)
...
func request_DocumentService_Get_0(ctx context.Context, marshaler runtime.Marshaler, client DocumentServiceClient, req *http.Request, pathParams map[string]string) (proto.Message, runtime.ServerMetadata, error) {
	var protoReq EntityRelationshipData
	var metadata runtime.ServerMetadata
	...
	val, ok = pathParams["owner_identity"]
	if !ok {
		return nil, metadata, status.Errorf(codes.InvalidArgument, "missing parameter %s", "owner_identity")
	}

	protoReq.OwnerIdentity, err = runtime.String(val)

	if err != nil {
		return nil, metadata, status.Errorf(codes.InvalidArgument, "type mismatch, parameter: %s, error: %v", "owner_identity", err)
	}

	if err := runtime.PopulateQueryParameters(&protoReq, req.URL.Query(), filter_DocumentService_Get_0); err != nil {
		return nil, metadata, status.Errorf(codes.InvalidArgument, "%v", err)
	}
