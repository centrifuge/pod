...
func TestHandler_RequestDocumentSignature_AlreadyExists(t *testing.T) {
	...
	assert.NoError(t, err)
	resp, err := handler.ReceiveDocumentSignatureRequest(ctxh, &p2ppb.SignatureRequest{Document: &cd}, did)
	assert.Nil(t, err, "must be nil")
...
func TestHandler_SendAnchoredDocument(t *testing.T) {
	...
	did, err = contextutil.AccountDID(ctxh)
	assert.NoError(t, err)
	resp, err = handler.ReceiveDocumentSignatureRequest(ctxh, &p2ppb.SignatureRequest{Document: &ncd}, did)
	assert.Nil(t, err)
	assert.NotNil(t, resp)
