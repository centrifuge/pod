...
func (m *mockModel) Author() identity.DID {
	args := m.Called()
...
func TestDefaultProcessor_PrepareForAnchoring(t *testing.T) {
	...
	model.sigs = append(model.sigs, sig)
	srv = &testingcommons.MockIdentityService{}
	srv.On("ValidateSignature", identity.NewDIDFromBytes(did), sig.PublicKey, sig.Signature, sr, tm).Return(errors.New("validation failed")).Once()
...
func (m mockRepo) GetAnchor(anchorID anchors.AnchorID) (docRoot anchors.DocumentRoot, anchoredTime time.Time, err error) {
...
func TestDefaultProcessor_SendDocument(t *testing.T) {
	...
	dp.identityService = srv
	repo := mockRepo{}
	repo.On("GetAnchor", aid).Return(nil, time.Now(), errors.New("error"))
	dp.anchorRepository = repo
	...
	dp.identityService = srv
	repo = mockRepo{}
	repo.On("GetAnchor", aid).Return(dr, time.Now(), nil).Once()
	dp.anchorRepository = repo
	err = dp.SendDocument(ctxh, model)
	model.AssertExpectations(t)
	srv.AssertExpectations(t)
