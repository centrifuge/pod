...
func TestDefaultProcessor_PrepareForSignatureRequests(t *testing.T) {
	...
	assert.True(t, crypto.VerifyMessage(self.Keys[identity.KeyPurposeSigning].PublicKey, cd.SigningRoot, sig.Signature, crypto.CurveSecp256K1))
}
...
func TestDefaultProcessor_PrepareForAnchoring(t *testing.T) {
	...
	assert.Nil(t, err)
	s, err := crypto.SignMessage(c.Keys[identity.KeyPurposeSigning].PrivateKey, cd.SigningRoot, crypto.CurveSecp256K1)
