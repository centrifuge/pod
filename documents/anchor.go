...
func AnchorDocument(ctx *header.ContextHeader, model Model, proc anchorProcessor, updater updaterFunc) (Model, error) {
	...
	if err != nil {
		return nil, errors.NewTypeError(ErrDocumentAnchoring, fmt.Errorf("failed to prepare document for signatures: %v", err))
	}
	...
	if err != nil {
		return nil, errors.NewTypeError(ErrDocumentAnchoring, fmt.Errorf("failed to collect signatures: %v", err))
	}

	err = updater(id, model)
	if err != nil {
		return nil, errors.NewTypeError(ErrDocumentAnchoring, err)
	}

	err = proc.PrepareForAnchoring(model)
	if err != nil {
		return nil, errors.NewTypeError(ErrDocumentAnchoring, fmt.Errorf("failed to prepare for anchoring: %v", err))
	}
	...
	if err != nil {
		return nil, errors.NewTypeError(ErrDocumentAnchoring, fmt.Errorf("failed to anchor document: %v", err))
	}
	...
	if err != nil {
		return nil, errors.NewTypeError(ErrDocumentAnchoring, fmt.Errorf("failed to send anchored document: %v", err))
	}
