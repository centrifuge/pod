...
type anchorProcessor interface {
	PrepareForSignatureRequests(ctx *ContextHeader, model Model) error
	RequestSignatures(ctx *ContextHeader, model Model) error
	PrepareForAnchoring(model Model) error
	AnchorDocument(ctx *ContextHeader, model Model) error
	SendDocument(ctx *ContextHeader, model Model) error
}
...
// to collaborators
func AnchorDocument(ctx *ContextHeader, model Model, proc anchorProcessor, updater updaterFunc) (Model, error) {
