...
func toClientAttachments(atts []*BinaryAttachment) []*clientpb.BinaryAttachment {
	catts := make([]*clientpb.BinaryAttachment, len(atts), len(atts))
	for i, att := range atts {
		...
		catts[i] = &clientpb.BinaryAttachment{
			Name:     att.Name,
...
func toP2PAttachments(atts []*BinaryAttachment) []*invoicepb.BinaryAttachment {
	patts := make([]*invoicepb.BinaryAttachment, len(atts), len(atts))
	for i, att := range atts {
		patts[i] = &invoicepb.BinaryAttachment{
			Name:     att.Name,
			FileType: att.FileType,
			Size:     att.Size,
			Data:     att.Data,
...
func fromClientAttachments(catts []*clientpb.BinaryAttachment) (atts []*BinaryAttachment, err error) {
	atts = make([]*BinaryAttachment, len(catts), len(catts))
	for i, att := range catts {
...
func fromP2PAttachments(patts []*invoicepb.BinaryAttachment) []*BinaryAttachment {
	atts := make([]*BinaryAttachment, len(patts), len(patts))
	for i, att := range patts {
		atts[i] = &BinaryAttachment{
			Name:     att.Name,
			FileType: att.FileType,
			Size:     att.Size,
...
func toClientLineItems(items []*LineItem) []*clientpb.InvoiceLineItem {
	citems := make([]*clientpb.InvoiceLineItem, len(items), len(items))
	for i, item := range items {
		...
		citems[i] = &clientpb.InvoiceLineItem{
...
func toP2PLineItems(items []*LineItem) ([]*invoicepb.InvoiceLineItem, error) {
	pitems := make([]*invoicepb.InvoiceLineItem, len(items), len(items))
	for i, item := range items {
		...
		pitems[i] = &invoicepb.InvoiceLineItem{
			ItemNumber:              item.ItemNumber,
			Description:             item.Description,
			SenderPartNo:            item.SenderPartNo,
			PricePerUnit:            decs[0],
...
func fromClientLineItems(citems []*clientpb.InvoiceLineItem) ([]*LineItem, error) {
	items := make([]*LineItem, len(citems), len(citems))
...
func fromP2PLineItems(pitems []*invoicepb.InvoiceLineItem) ([]*LineItem, error) {
	items := make([]*LineItem, len(pitems), len(pitems))
...
func toP2PPaymentDetails(details []*PaymentDetails) ([]*invoicepb.PaymentDetails, error) {
	pdetails := make([]*invoicepb.PaymentDetails, len(details), len(details))
	for i, detail := range details {
		...
		pdetails[i] = &invoicepb.PaymentDetails{
			Id:                    detail.ID,
			DateExecuted:          detail.DateExecuted,
			Payee:                 dids[0],
			Payer:                 dids[1],
...
func fromClientPaymentDetails(cdetails []*clientpb.PaymentDetails) ([]*PaymentDetails, error) {
	details := make([]*PaymentDetails, len(cdetails), len(cdetails))
	for i, detail := range cdetails {
...
func fromP2PPaymentDetails(pdetails []*invoicepb.PaymentDetails) ([]*PaymentDetails, error) {
	details := make([]*PaymentDetails, len(pdetails), len(pdetails))
	for i, detail := range pdetails {
...
func toP2PTaxItems(items []*TaxItem) ([]*invoicepb.InvoiceTaxItem, error) {
	pitems := make([]*invoicepb.InvoiceTaxItem, len(items), len(items))
	for i, item := range items {
		...
		pitems[i] = &invoicepb.InvoiceTaxItem{
			ItemNumber:        item.ItemNumber,
			InvoiceItemNumber: item.InvoiceItemNumber,
			TaxAmount:         decs[0],
			TaxRate:           decs[1],
...
func fromClientTaxItems(citems []*clientpb.InvoiceTaxItem) ([]*TaxItem, error) {
	items := make([]*TaxItem, len(citems), len(citems))
	for i, item := range citems {
...
func fromP2PTaxItems(pitems []*invoicepb.InvoiceTaxItem) ([]*TaxItem, error) {
	items := make([]*TaxItem, len(pitems), len(pitems))
	for i, item := range pitems {
