...
// AccountCanUpdate checks if the account can update the document.
func (i *Invoice) AccountCanUpdate(updated documents.Model, account identity.DID) error {
	...
	err := i.CoreDocument.AccountCanUpdate(newInv.CoreDocument, account, i.DocumentType())
	if err != nil {
		return err
	}
	...
	rules := i.CoreDocument.TransitionRulesFor(account)
